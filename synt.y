...
;
NW : FW | WW ; // SOIT AVEC OP LOGIQUE  SOIT OP SIMPLE

FW : EW expC PF {
   cond2=qc; 
   if(l==3){logic=true;}
   else{logic=false;} // dans tous les cas la deuxieme cond ne change pas 
  quadL(l,j,"","","",logic);
  sprintf(tmp,"%d",qc); 
...
;
EW :  mc_WHILE PO expC op_logique
 { cond1=qc;
 quadL(l,j,"","","",logic);
ajour_quad(cond1,2,cst);
ajour_quad(cond1,3,cst2);}
|mc_WHILE PO op_NOT {l=3;} //logic pour le cas du OR de la premiere cond
;
